...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if current_ant.is_container == True and current_ant.can_contain(self) == True:
                current_ant.ant_contained = self
            elif self.is_container == True and self.can_contain(current_ant) == True:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        for _ in range(self.lower_bound):
            next_place = p.entrance
            if next_place != None:
                p = next_place
        while p.is_hive != True and i <= self.upper_bound:
            randomBee = random_bee(p.bees)
            if randomBee != None:
                return randomBee
            p = p.entrance
            i += 1
    ...
    def reduce_health(self, amount):
        ...
        if bees_copy != None:
            for bee in bees_copy:
                bee.reduce_health(amount + additional_damage)
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt will select a random Bee from its place and deal damage to the Bee equal to the Bee's health, eating it whole."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower, which is a subclass of ThrowerAnt that is more costly and waterproof."""
    ...
    def throw_at(self, target):
        ...
        if target is not None:
            if target.slow_turns != 0:
                target.slow_turns = self.slowdown_turns
            else:
                target.slow_turns = self.slowdown_turns
                original_action = target.action
                
                def action_slowed(gamestate):
                    if target.slow_turns > 0:
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target.has_been_scared == False:
            target.scare(self.scared_length)
        # END Problem EC 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while place.is_hive == False:
            bees_copy = place.bees[:]
            if bees_copy != None:
                for bee in bees_copy:
                    insects_in_front[bee] = distance
                    
            ant = place.ant
            if ant is not None and ant is not self:
                insects_in_front[ant] = distance
                if ant.is_container == True: 
                    if ant.ant_contained is not None and ant.ant_contained is not self:
                        insects_in_front[ant.ant_contained] = distance
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.has_been_scared == True and self.scared_turns > 0:
            if self.place.is_hive != True:
                if self.slow_turns == 0 or gamestate.time % 2 == 0:
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        self.has_been_scared, self.scared_turns = True, length
        # END Problem EC 2
