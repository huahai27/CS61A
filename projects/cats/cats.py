...
def about(subject):
    ...
    def contain(paragraph):
        ...
        para_no_punc = remove_punctuation(paragraph_lower)
        words = para_no_punc.split()
        for word in words:
            for sub in subject:
                if word == sub:
                    return True
...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 and len(source_words) != 0:
        return 0.0
    loops = min(len(typed_words), len(source_words))
    correct = 0
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    groups_divided_by_5 = len(typed) / 5 
    WPM = groups_divided_by_5 * 60 / elapsed
    return WPM
    # END PROBLEM 4
...
def time_per_word(words, timestamps_per_player):
    ...
    for timestamp in tpp:
        ...
        for i in range(1, len(timestamp)):
            player += [timestamp[i] - timestamp[i - 1]]
        times += [player]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for w in word_indices:
        ...
        for p in player_indices:
            time += [get_time(times, p, w)]
        fastest_player = time.index(min(time))
        log[fastest_player] += [words[w]]
    return log
    # END PROBLEM 10
