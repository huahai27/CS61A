...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    k, total, SowSad = 1, 0, False
    while k <= num_rolls:
        pt = dice()
        if pt == 1:
            SowSad = True
        total, k = total + pt, k + 1
    if SowSad:
        total = 1
    return total
    # END PROBLEM 1
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    if not num_rolls:
        pt = boar_brawl(player_score, opponent_score)
    else:
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    num, new = num_factors(score), score
    if num == 3 or num ==4:
        new += 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score0 < goal:
        score1 = 0
        same = strategy(score0, score1)
        while score1 < goal:
            score1 += 1
            if same != strategy(score0, score1):
                return False
        score0 += 1
    return True
    # END PROBLEM 7
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    score_sus = sus_update(0, score, opponent_score) - score
    if score_sus >= threshold:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
