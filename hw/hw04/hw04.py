...
def balanced(m):
    ...
    "*** YOUR CODE HERE ***"
    l, r = left(m), right(m)
    end_l, end_r = end(l), end(r)
    if is_mobile(end_l) and not balanced(end_l):
        return False
    elif is_mobile(end_r) and not balanced(end_r):
        return False
    torque_l = total_mass(end_l) * length(l)
    torque_r = total_mass(end_r) * length(r)
